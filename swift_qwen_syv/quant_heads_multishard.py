# Derived from prepare/quant_heads_stream.py in syv-ai/qwen38-27b-rtx3090
# (Apache-2.0, Copyright syv-ai contributors). Modifications: multi-shard
# lm_head/embed handling, per this file's docstring.
"""Requantize lm_head, embed_tokens and the MTP module to int8 (group-128,
symmetric) compressed-tensors pack-quantized, in place — for checkpoints whose
lm_head and embed_tokens live in DIFFERENT safetensors shards.

Derived from syv-ai/qwen38-27b-rtx3090's prepare/quant_heads_stream.py (same
math, same output tensors, same config.json group edits). Upstream's script
asserts `wm[emb_key] == wm[lm_key]` ("lm_head and embed_tokens live in
different shards") because it rewrites ONE shard in a single streaming pass.
TheUnderscore/Swift-Qwen3.8-27b-W4A16-AWQ splits them: lm_head in shard 7,
model.language_model.embed_tokens.weight in shard 1, the MTP module in
model-nonquant.safetensors. This variant does one streaming pass per head
shard; everything else matches upstream exactly.

Run inside the syv container (torch + safetensors + compressed_tensors), from
the model dir's parent, as:

  python quant_heads_multishard.py /app/models/Swift-Qwen3.8-27B-W4A16-syv

Backups: <shard>.bak-orig for each rewritten shard, .bak-quant for the index
and config.json — same convention as upstream.
"""
import copy
import json
import os
...
# ---- lm_head and embed_tokens, one streaming pass over EACH of their shards ----
for key, scale_dtype in ((lm_key, torch.float16), (emb_key, torch.bfloat16)):
    shard = wm[key]
    print(f"quantizing {key} in {shard}")
    ...
    add = {
        ...
        base + ".weight_scale": scale.to(scale_dtype),
        base + ".weight_shape": torch.tensor([out_f, in_f], dtype=torch.int64),
    }
    del w, packed, scale

    print(f"rewriting {shard} (streaming)")
    tmp = d + shard + ".tmp"
    stream_rewrite(d + shard, tmp, drop={key}, add=add)
...
# Drop ignore entries naming exactly the tensors now quantized: this
# checkpoint's ignore list has "lm_head" (embed_tokens and mtp.* are not in
# it — they were never Linear-target candidates).
qc["ignore"] = [
    i for i in qc["ignore"]
    if i != "lm_head" and i != "lm_head.weight" and i not in MTP_LINEARS
]
qc["config_groups"]["group_1"] = group(HEAD_BITS, ["re:.*lm_head$"])
qc["config_groups"]["group_2"] = group(HEAD_BITS, ["re:.*embed_tokens$"])
qc["config_groups"]["group_3"] = group(MTP_BITS, ["re:^mtp\\..*"])
